...
def move_product_image(request):
    image_dir = os.path.join(BASE_DIR, 'static/media/default/img/')
    files = os.listdir(image_dir)
    current_user = request.user
    user_destination = os.path.join(BASE_DIR, 'static/media/{}/img/products/'.format(current_user.email))
    for file in files:
        if str(file) != "default.png":
            src = os.path.join(image_dir, file)
            dst = os.path.join(user_destination, file)
...
def get_product_url(model, user_email):
    product_url = '{}/img/products/'.format(user_email)
    dir_path, filename = change_image_path(model.get_image_url())
    return os.path.join(product_url, filename)
